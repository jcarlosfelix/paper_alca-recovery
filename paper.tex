...
\section{Introduction}
    ...
    to solve them \cite{ontiveros2018high}. % You can add a comment on why it  
    % requires more power, the main reason is the fitness evaluation function - M 
    One technique to manage this
    processing power is to use distributed computing % To manage the processing power need or demand? - M
    \cite{thain2005distributed} or the cloud resources 
    \cite{garcia2013there,eshratifar2019bottlenet}. This strategy
    allows adaptating the required computing power according to the problem's
    complexity.
    ...
    % Add here a few current works that use the cloud and other distributed technologies - M
    In this research,
    we present a distributed algorithm built as a native cloud solution %, totally % fully, but this is hard to maintain - M
    % distributed
    ...
    in the cloud. 
    % The paragraph above is to repetitive, consider removing -  M
    ...
    In previous work, we introduced an algorithm inspired by the biological % Add the reference - M
    ...
    We previously used to generate a new random % In a previous work (reference) - M
    ...
    % The main contribution is not this, is the restarting method? - M 
    We improve the algorithm behavior by solving
    population extinction caused by nature's pressure in the Animal Life Cycle % This is more a contribution - M
    ...
    We present the description of the following sections in our paper. First, we % This paper is organized as follows - M 
...
\section{Proposal}
    ...
    In previous work, we presented an algorithm inspired by nature, where we % Reference
    % used observation, analysis, and abstraction to 
    ...
    \subsection{Algorithm Model}
        ...
        % Start by stating the similarities with a GA, in the sense that 
        % all individuals have a genotype (chromosome)that is a list of values.  - M
    ...
    \subsection{Proposed Solution}
        ...
        % Where are the elites explained? - M
        ...
        an improved distribution. % Why is better? Explain. - M
        ...
        % Move the explanation of Elite to where we use the term for the first time - M
        ...
        strategy obtained the best results. % All this goes before they are mentioned. - M 
         ...
         % \subsubsection{How does it work?}  
            ...
            % You can use Elite now, we all now what it is. - M
